...
public class JwtAuthFilter extends OncePerRequestFilter{
	...
	//se inyecta una SecretKey que se utiliza para firmar y verificar los tokens JWT.
		public JwtAuthFilter(SecretKey secretKey) { 
			this.secretKey = secretKey;
		}

		@Override
		//Este método se ejecuta para cada solicitud entrante. Extrae el token JWT de la solicitud, lo valida y, si es válido, establece la autenticación en el contexto de seguridad de Spring.
		protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
			...
			try {
				...
				if (token != null && validateToken(token)) {
					String usuario = extractUsernameFromToken(token);
		...
		private String extractJwtFromRequest(HttpServletRequest request) { //extrae el token JWT de la cabecera de autorización de la solicitud.
		...
		private boolean validateToken(String token) { //Este método valida el token JWT, verificando su firma y su fecha de vencimiento.
			try {
